 ...
 * For example, for MP1 we test that your summary comparator and filter function work as expected.
 ...
 * For example, for MP1, `testSummaryView` checks to make sure that the right number of summaries is loaded when the
...
public final class MP1Test {
  ...
  public static class IntegrationTests {
    ...
    public void testSummaryView() {
      ...
      onView(withId(R.id.recycler_view)).perform(RecyclerViewActions.scrollToPosition(72));
      onView(withRecyclerView(R.id.recycler_view).atPosition(72))
          .check(matches(hasDescendant(withText("CS 599: Thesis Research"))));
    }
